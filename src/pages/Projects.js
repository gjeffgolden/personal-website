...
export default function Projects() {
    ...
    const displayProjects = () => {
        return projects.map(project => <ProjectCard title={project.title} screenshot={project.screenshot} github={project.github} description={project.description} />)
    }

    return (
        <Container 
            maxWidth='xl' 
            style={{height: "60%", width: "80%", backgroundColor: "rgb(0, 0, 0, .3", marginTop: '3rem', display: 'flex', flexFlow: 'row', justifyContent: 'space-between', alignItems: 'center'}}
        >
            {displayProjects()}
        </Container>
    )
